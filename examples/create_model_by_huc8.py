#!/usr/bin/env python3
"""Queue a whole-basin HUC8 SWAT+ model: POST /api/model-settings-huc8 (Effective Pro required)."""
import json
import os
import sys
...
HUC8 = (os.environ.get("SWATGENX_EXAMPLE_HUC8") or "04100004").strip().zfill(8)
...
def main() -> int:
    ...
    r = requests.post(f"{BASE}/api/model-settings-huc8", headers=headers, json=payload, timeout=120)
