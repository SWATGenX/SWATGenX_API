...
`GET /api/task_status/<task_id>` tells you when the run reached **SUCCESS**, but it **does not** currently echo the download URL in JSON — treat the **email link** (or dashboard) as the hand-off for the token URL. A future API could return a fresh signed URL in JSON; until then, automation usually stores `SWATGENX_DOWNLOAD_URL` in a secret after you copy it once from email.
